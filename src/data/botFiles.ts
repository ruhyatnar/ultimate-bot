...
export const BOT_FILES: BotFileDefinition[] = [
  ...
  },
  {
    ...
    name: 'config.py',
    category: 'root',
    description: 'Central configuration manager, strategy presets (scalping, day, swing), and strict startup validation.',
    enhancements: [
      ...
      'Mathematical constraint validation (TP > SL, callback < activate, threshold 1-5, etc.)'
    ],
    content: CONFIG_PY,
  },
  {
  ...
  },
  {
    ...
    category: 'strategies',
    description: '5-factor confluence engine: HTF dual EMA, Break of Structure, FVG imbalance, CVD order flow, POC.',
    enhancements: [
      '60s kline cache (bounded FIFO) cuts REST weight usage by ~90% per scan loop',
      'True ATR series with fallback flooring so stops are never zero-width',
      'Uses last CLOSED candles for FVG to avoid repainting signals',
      'Per-symbol DEBUG logging of every factor for the web monitor'
    ],
  ...
  },
  {
    ...
    name: 'trade_logic.py',
    category: 'trade',
    description: 'Core execution controller: signal loop, entries, trailing/breakeven exits, reconciliation, daily reports.',
    enhancements: [
      ...
      'Fee-aware breakeven lock at entry +0.25%; ATR trailing stop with ratchet',
