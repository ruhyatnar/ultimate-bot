...
def main():
    ...
    if lock_is_held():
        ...
        print("       Stop it first (pm2 delete ultimate-bot / kill) and re-run.")
        return 2
    ...
    try:
        ...
        with open(engine_log, "w") as f:
            engine = subprocess.Popen(
                [PYTHON, "main.py"], cwd=ROOT, env=env,
            ...
            try:
                ...
                try:
                    ...
                    if not {"orders", "active_trades", "risk_state"} <= tables:
                        return False
                    row = conn.execute(
                        "SELECT value FROM risk_state WHERE key='paper_balance'").fetchone()
        ...
        if not booted:
            print("FAIL: engine did not create a usable database in time. Log tail:")
        ...
        try:
            ...
            rejected = second.returncode == 1 and "Another instance is already running" in second.stdout
            check("second instance rejected (exit 1 + message)", rejected,
                  f"rc={second.returncode}")
        except subprocess.TimeoutExpired:
            check("second instance rejected (exit 1 + message)", False, "timed out")
        ...
        with open(monitor_log, "w") as f:
            monitor = subprocess.Popen(
                [PYTHON, "status.py", "--web", str(port)], cwd=ROOT, env=env,
                stdout=f, stderr=subprocess.STDOUT,
            )
        ...
        def api_ok():
            try:
                with urllib.request.urlopen(
                        f"http://127.0.0.1:{port}/api/status", timeout=3) as resp:
        ...
        if not wait_for(api_ok, 30):
            ...
            check("stats reconcile (closed == W + L + B)", consistent,
                  f"{wins}W/{losses}L/{breakevens}B closed={closed}")
            running = "RUNNING" in str(data.get("process", ""))
            check("monitor reports engine RUNNING", running, str(data.get("process")))
            ...
            matches_db = (api_closed, api_wins, api_losses) == (db_closed, db_wins, db_losses)
        ...
        exited = wait_for(lambda: engine.poll() is not None, SHUTDOWN_TIMEOUT, 0.25)
        check("engine exited after SIGTERM", exited,
              f"rc={engine.poll()}" if exited else "still alive")
        if exited:
            check("clean exit code 0", engine.returncode == 0, f"rc={engine.returncode}")
        else:
        ...
        with open(engine_log) as f:
            engine_text = f.read()
        check("log contains 'Shutdown complete.'", "Shutdown complete." in engine_text)
        check("no 'Task was destroyed' warnings", "Task was destroyed" not in engine_text)
